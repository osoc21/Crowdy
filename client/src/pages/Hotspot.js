...
const Hotspot = () => {
  ...
  const changeFavourites = hotspot => {
    ...
    favourites.includes(hotspot) ? tmp.splice(favourites.indexOf(hotspot), 1) : tmp.push(hotspot);
    setFavourites(tmp);
  };

  const navOptions = [
    { name: 'favourite', action: changeFavourites }
  ];
  ...
  return (
    <section className={styles.container}>
      ...
      <ul className={styles.services}>
        {data.services.map(item => <div className={styles[`btn__${item}`]} key={item}></div>)}
      </ul>
      <p className={styles.crowdedness}>{crowdedness[data.crowdedness - 1]} ({data.crowdedness})</p>
